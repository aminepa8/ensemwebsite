




		<!-- Header
		============================================= -->
		<header   id="header" class="full-header sticky-on-upscroll ">

			<div id="header-wrap">

				<div class="container clearfix">

					<div id="primary-menu-trigger"><i class="icon-reorder"></i></div>

					<!-- Logo
					============================================= -->

						<div id="logo">

							<a  href="#" class="standard-logo"><img class="dark_ensem" src="includes/images/ensem_logo.png" alt="ensem_logo.png"></a>
						</div>

				<!-- #logo end -->

					<!-- Primary Navigation
					============================================= -->
					<nav id="primary-menu" class="style-3 not-dark">


						<ul>
				<li><a href="#"><div>Accueil</div></a>
					<ul>
						<li><a href="#"><div>Menu 1</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 2</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 3</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 4</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 5</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>




					</ul>
				</li>
				<li><a href="#"><div>L’Ecole</div></a>
					<ul>
						<li><a href="#"><div>Menu 1</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 2</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 3</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 4</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 5</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>




					</ul>
				</li>
				<li><a href="#"><div>Recherche</div></a>
					<ul>
						<li><a href="#"><div>Menu 1</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 2</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 3</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 4</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 5</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>




					</ul>
				</li>
				<li><a href="#"><div>Espace-Entreprise</div></a>
					<ul>
						<li><a href="#"><div>Menu 1</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 2</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 3</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 4</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 5</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>




					</ul>
				</li>
				<li><a href="#"><div>Espace-Etudiant</div></a>
					<ul>
						<li><a href="#"><div>Menu 1</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 2</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 3</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 4</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 5</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>




					</ul>
				</li>
				<li><a href="#"><div>Contact</div></a>
					<ul>
						<li><a href="#"><div>Menu 1</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 2</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 3</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 4</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>
						<li><a href="#"><div>Menu 5</div></a>
							<ul>
								<li><a href="#"><div>Sub-menu 1</div></a></li>
								<li><a href="#"><div>Sub-menu 2</div></a></li>
								<li><a href="#"><div>Sub-menu 3</div></a></li>
								<li><a href="#"><div>Sub-menu 4</div></a></li>
							</ul>
						</li>




					</ul>
				</li>
			</ul>



						<!-- Top Search
						============================================= -->
						<div id="top-search">
							<a href="#" id="top-search-trigger"><i class="icon-search3"></i><i class="icon-line-cross"></i></a>
							<form action="search.html" method="get">
								<input type="text" name="q" class="form-control" value="" placeholder="Tapez &amp; Appuyez sur Entrée.." autocomplete="off">
							</form>
						</div><!-- #top-search end -->

					</nav><!-- #primary-menu end -->

				</div>

			</div>

		</header><!-- #header end -->
